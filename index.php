...
include 'oauth2/zwoauth.class.php';
...
$PAGE->set_url("$CFG->httpswwwroot/loginoauth/index.php");
$PAGE->set_context($context);
$PAGE->set_pagelayout('loginpage');
...
} else if (!empty($SESSION->wantsurl) && file_exists($CFG->dirroot.'/loginoauth/weblinkauth.php')) {
    // Handles the case of another Moodle site linking into a page on this site
    //TODO: move weblink into own auth plugin
//    include($CFG->dirroot.'/loginoauth/weblinkauth.php');
...
//        echo $OUTPUT->footer();
    ...
    if ($user) {
        ...
        $tokenresult = $oauth->grantAccessToken();

        zzwu_set_access_token($tokenresult['access_token'], $USER->id);
//        echo json_encode($tokenresult);
        $redirect_uri = $tokenresult['redirect_uri'];
        $preg='|^http://|';  //正则，匹配以http://开头的字符串
        if(!preg_match($preg,$redirect_uri)) {  //如果不能匹配
...
//        file_get_contents($redirect_uri, false, $context);
//        return $result;
...
//    send_post('http://www.qianyunlai.com', $post_data);
        header('Location:'.$redirect_uri.'?accesstoken='.$tokenresult['access_token']);
//        header('Location:'.$redirect_uri);
        die();
...
//                    $passwordchangeurl = $CFG->httpswwwroot.'/loginoauth/change_password.php';
//                }
//            } else {
//                $passwordchangeurl = $CFG->httpswwwroot.'/loginoauth/change_password.php';
//            }
...
//            $referer != $CFG->wwwroot &&
//            $referer != $CFG->wwwroot . '/' &&
//            $referer != $CFG->httpswwwroot . '/loginoauth/' &&
//            strpos($referer, $CFG->httpswwwroot . '/loginoauth/?') !== 0 &&
//            strpos($referer, $CFG->httpswwwroot . '/loginoauth/index.php') !== 0) { // There might be some extra params such as ?lang=.
...
//    }
//    redirect(new moodle_url('/loginoauth/index.php'));
//}
...
//    $logout = new single_button(new moodle_url($CFG->httpswwwroot.'/loginoauth/logout.php', array('sesskey'=>sesskey(),'loginpage'=>1)), get_string('logout'), 'post');
//    $continue = new single_button(new moodle_url($CFG->httpswwwroot.'/loginoauth/index.php', array('cancel'=>1)), get_string('cancel'), 'get');
